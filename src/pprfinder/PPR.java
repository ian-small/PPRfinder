...
public class PPR {
    ...
    public void addMotifAlignment(String motif_data, String motif_type) {
        ...
        System.out.println("Could not find motif " + motif_data + " in " + id);
    }
    ...
    public void mergeMotifs(){
        ...
        for (int i=1; i<motifs.size(); i++){
            ...
            if (m1_coverage >= Motif.getCanonicalLength(m1)) continue;
            if (m2_coverage >= Motif.getCanonicalLength(m2)) continue;
    ...
    public String getBestString() {
        ...
        double best_valid_score = -100.0;
        
        for (String_of_Beads sob : sobs) {
            
            if (sob.getScore() > best_score) {
                best_sob = sob;
                best_score = best_sob.getScore();
            }
            if (sob.isValid() && sob.getScore() > best_valid_score) {
                best_valid_sob = sob;
                best_valid_score = best_valid_sob.getScore();
            }
        }
        
        if (best_valid_sob != null) {
            best_sob = best_valid_sob;
        }
    ...
    public double FFT(int wavelength, int phase) {
        ...
        for (int i=0; i< transforms.length; i++) {
            transforms[i] = new ArrayList();
        }
        
        int index = phase;
        
        while (index < sequence.length()) {
            transforms[(index-phase)%wavelength].add(sequence.substring(index,++index));
        }
